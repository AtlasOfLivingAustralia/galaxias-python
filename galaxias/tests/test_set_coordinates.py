...
def test_no_dataframe():
    occ_dwca = galaxias.dwca(occurrences='data_for_testing/occurrences_dwc_clean.csv',working_dir='dwca_data_occ',print_notices=False)
...
def test_set_coordinates_no_arguments():
    ...
    with pytest.raises(Exception) as e_info:
        occ_dwca.set_datetime(dataframe=df)
...
def test_not_both_decimalLatitude_decimalLongitude():
    ...
    occ_dwca = galaxias.dwca(occurrences=df)
    with pytest.raises(Exception) as e_info:
        occ_dwca.set_coordinates(dataframe=df)
