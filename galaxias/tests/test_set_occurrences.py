...
def test_set_occurrences_no_arguments():
    ...
    with pytest.raises(Exception) as e_info:
        occ_dwca.set_occurrences(dataframe=df)
    assert "No Darwin Core" in str(e_info.value)

def test_set_occurrences_occurrenceID_rename():
    df = pd.DataFrame({'id': ['1','2']})
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceID='id')
...
def test_set_occurrences_occurrenceStatus_rename():
    ...
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceStatus='status')
...
def test_set_occurrences_add_occurrenceIDs_random():
    df = pd.DataFrame({'species': ['Eolophus roseicapilla','Eolophus roseicapilla']})
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceID=True,random_id=True)
...
def test_set_occurrences_add_occurrenceIDs_sequential():
    df = pd.DataFrame({'species': ['Eolophus roseicapilla','Eolophus roseicapilla'],
                       ...
                       'decimalLongitude': [1.0,1.0]})
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceID=True,sequential_id=True)
    assert 'occurrenceID' in occ_dwca.occurrences.columns
...
def test_set_occurrences_add_occurrenceIDs_composite():
    df = pd.DataFrame({'species': ['Eolophus roseicapilla','Eolophus roseicapilla'],
                       ...
                       'catalogNumber': ['1','2']})
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceID=True,composite_id=['eventDate','catalogNumber'])
    assert 'occurrenceID' in occ_dwca.occurrences.columns
...
def test_set_occurrences_add_occurrenceIDs_composite_random():
    df = pd.DataFrame({'species': ['Eolophus roseicapilla','Eolophus roseicapilla'],
                       ...
                       'catalogNumber': ['1','2']})
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceID=True,composite_id=['eventDate','catalogNumber'],random_id=True)
    assert 'occurrenceID' in occ_dwca.occurrences.columns
...
def test_set_occurrences_add_occurrenceIDs_composite_random_last():
    df = pd.DataFrame({'species': ['Eolophus roseicapilla','Eolophus roseicapilla'],
                       ...
                       'catalogNumber': ['1','2']})
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceID=True,composite_id=['eventDate','catalogNumber'],random_id=True,add_random_id='last')
    assert 'occurrenceID' in occ_dwca.occurrences.columns
...
def test_set_occurrences_add_occurrenceIDs_composite_sequential():
    df = pd.DataFrame({'species': ['Eolophus roseicapilla','Eolophus roseicapilla'],
                       ...
                       'catalogNumber': ['1','2']})
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceID=True,composite_id=['eventDate','catalogNumber'],sequential_id=True)
    assert 'occurrenceID' in occ_dwca.occurrences.columns
...
def test_set_occurrences_add_occurrenceIDs_composite_sequential_last():
    df = pd.DataFrame({'species': ['Eolophus roseicapilla','Eolophus roseicapilla'],
                       ...
                       'catalogNumber': ['1','2']})
    occ_dwca = galaxias.dwca(occurrences=df)
    occ_dwca.set_occurrences(dataframe=df,occurrenceID=True,composite_id=['eventDate','catalogNumber'],sequential_id=True,add_sequential_id='last')
    assert 'occurrenceID' in occ_dwca.occurrences.columns
#'''